        ...
        stage('Docker Build') {
            ...
            steps {
                echo 'docker image cmd ${DOCKER_REGISTRY}'
                 def imageName = bat script: 'mvn help:evaluate -Dexpression=jkube.generator.name -q -DforceStdout -Ddocker.registry=${DOCKER_REGISTRY} -Djkube.namespace=${NAMESPACE} -Dbuild.number=${BUILD_NUMBER}'
